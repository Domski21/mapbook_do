...
def add_user(userlist:list) -> None:
    new_name:str=input('Proszę podać imię nowego uźytkownika ')
...
def update_user(userlist:list) -> None:
    user_to_find: str = input('Podaj imie osoby do aktualizacji: ')
    for user in userlist:
        if user['name'] == user_to_find:
            new_name: str = input('Proszę podać nowe imie ')
